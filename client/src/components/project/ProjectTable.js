...
const ProjectTable = ( { projects } ) => {
  ...
  return <Table striped className="border">
    <thead>
      <tr>
        <th> # </th>
    ...
    <tbody>
      { projects.length ? <>
        { projects.map( ( item, index ) => {
          ...
          let project_name = item.name.replace( / /g, "-" )  // replace ' ' with '-'
          return <tr key={ index }>
            <td>{ index + 1 }</td>
            <td className="clickable">
              <Link to={ `projects/${ project_name }/${ item._id }` }>{ item.name }</Link>
            </td>
            <td>{ description_short }</td>
            <td>{ moment( item.createdAt ).format( 'YYYY-MM-DD' ) }</td>
            <td>
              <i className="fas fa-trash-alt skyblue clickable" onClick={ () => handleDeleteProject( item._id ) } />
            </td>
          </tr>
        } ) }
      </>
        : <>
          <tr>
            <td colSpan="6" className="center"> No projects </td>
          </tr>
        </>
      }
    </tbody>
