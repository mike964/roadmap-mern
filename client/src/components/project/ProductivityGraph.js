...
const ProductivityGraph = ({ steps }) => {
  ...
  const getLastTenDays = () => {
    ...
    for (let i = 0; i < 14; i++) {
      let x = moment(yesterdayMinus14)
        .add(i + 1, 'd')
        .format('YYYY-MM-DD')
      let label = moment(yesterdayMinus14)
        .add(i + 1, 'd')
        .format('MMM DD')
      days[i] = x
      labels[i] = label
  ...
  useEffect(() => {
    ...
    if ((lastTenDays.length, steps.length)) {
      const completedStepsPerDay = lastTenDays.map((item) => {
        const completedSteps = steps.filter(
          (step) =>
            step.completed === true &&
            moment(step.completedAt).format('YYYY-MM-DD') === item,
        )
